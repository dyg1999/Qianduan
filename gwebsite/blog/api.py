...
from blog.models import Article, Userinfo, Lanmu, Pinglun, PayOrder, Like, Favourite
...
def add_article(request):
    ...
    # 保存文章
    new_airticle = Article(title=title)
    new_airticle.save()
    ...
    for img in range(0, len(imgList)):
        ...
        if 'http://' in src or 'https://' in src:
            ...
            image_name = datetime.datetime.now().strftime('%Y%m%d%H%M%S')+'-' + str(new_airticle.id) + \
                '-' + str(img) + '.' + \
    ...
    new_airticle.content = content
    new_airticle.describe = describe
    new_airticle.cover = cover
    new_airticle.belong = user_token[0].user
    new_airticle.save()
    return Response('ok')
...
def dygweb_group(request):
    ...
    if request.method == "POST":
        ...
        checkUser = userLoginAndPerm(token, permLsit)
        if checkUser != 'perm_pass':
            return Response(checkUser)
...
def userLoginAndPerm(token, permLsit):
    user_token = Token.objects.filter(key=token)
    if user_token:
        user = user_token[0].user
        for perm_str in permLsit:
